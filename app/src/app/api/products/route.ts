...
const FIELDS = ['name', 'description', 'price', 'category', 'emoji', 'image_url', 'sku', 'track_stock', 'stock_qty', 'in_shop', 'in_pos', 'active', 'sort'] as const
...
export async function POST(req: Request) {
  ...
  const { data, error } = await admin.from('products').insert({ tenant_id: g.tenantId, ...clean(body) }).select('id').single()
...
export async function PATCH(req: Request) {
  ...
  const admin = createAdminSupabase()
  const { error } = await admin.from('products').update(clean(body)).eq('id', body.id).eq('tenant_id', g.tenantId)
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
...
export async function DELETE(req: Request) {
  ...
  const admin = createAdminSupabase()
  // Soft-delete (keep history clean): mark inactive.
  const { error } = await admin.from('products').update({ active: false }).eq('id', id).eq('tenant_id', g.tenantId)
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
  return NextResponse.json({ ok: true })
}
