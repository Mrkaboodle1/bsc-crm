// /api/coach-replies — mark read / delete a coach reply (owner/manager).
...
export async function PATCH(req: Request) {
  ...
  const { error } = await admin.from('coach_replies').update({ is_read: b.is_read !== false }).eq('id', b.id).eq('tenant_id', g.tenantId)
...
export async function DELETE(req: Request) {
  ...
  const admin = createAdminSupabase()
  const { error } = await admin.from('coach_replies').delete().eq('id', id).eq('tenant_id', g.tenantId)
  if (error) return NextResponse.json({ error: error.message }, { status: 400 })
  return NextResponse.json({ ok: true })
}
