...
export function Storefront({ products = PRODUCTS, businessName = 'the team' }: { products?: Product[]; businessName?: string }) {
  ...
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {buy && <BuyModal product={buy} businessName={businessName} onClose={() => setBuy(null)} />}
    </div>
  )
}

function BuyModal({ product, businessName, onClose }: { product: Product; businessName: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        {state === 'done' ? (
          <div className="p-6 text-center"><div className="text-4xl mb-2">🎪</div><p className="font-bold text-zinc-900">Order received!</p><p className="text-sm text-zinc-600 mt-1">{businessName} will confirm your <strong>{product.name}</strong> and payment shortly.</p></div>
        ) : (
